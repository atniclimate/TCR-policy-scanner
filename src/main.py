...
def list_programs(programs: list[dict]) -> None:
    """Print the tracked program inventory."""
    print(f"\nTracked Programs ({len(programs)}):")
    print("-" * 70)
    for p in programs:
        priority_tag = f"[{p['priority'].upper()}]"
        print(f"  {priority_tag:10s} {p['name']:35s} ({p['agency']})")
        print(f"{'':12s} Lever: {p['advocacy_lever']}")
    print()


def dry_run(config: dict, sources: list[str]) -> None:
...
async def run_scan(config: dict, programs: list[dict], sources: list[str]) -> list[dict]:
    """Execute scrapers and return all collected items."""
    all_items = []
...
def run_pipeline(config: dict, programs: list[dict], sources: list[str], report_only: bool = False) -> None:
    """Run the full scan-score-detect-report pipeline."""
    ...
    paths = reporter.generate(scored, changes)
    print(f"\nScan complete.")
...
def main() -> None:
    ...
    parser.add_argument("--programs", action="store_true", help="List tracked programs")
    ...
    if args.dry_run:
        dry_run(config, sources)
        return

    run_pipeline(config, programs, sources, report_only=args.report_only)

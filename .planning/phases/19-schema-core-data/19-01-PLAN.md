...
<task type="auto">
  ...
  <action>
    ...
    **src/packets/docx_engine.py** (2 instances):
    ...
    - Line ~174 docstring: "FY26 program overview header" -> f"{FISCAL_YEAR_SHORT} program overview header" -- actually this is a docstring so use generic wording: "current fiscal year program overview header"
...
<task type="auto">
  ...
  <action>
    ...
    **src/packets/context.py** -- Add `vulnerability_profile` field to TribePacketContext:
    ...
    After the `hazard_profile` field (line ~63), add:
    ```python
    ...
    ```
    Vulnerability profile (v1.4):
        vulnerability_profile: Composite vulnerability data (NRI expanded,
            SVI, climate, flood, infrastructure).
    ```

    **tests/test_paths.py** -- Create new test file (or add to existing if it exists):
...
4. `python -c "from src.packets.context import TribePacketContext; t = TribePacketContext(tribe_id='x', tribe_name='X'); assert 'vulnerability_profile' in t.to_dict()"` -- confirms field exists in serialization
</verification>
...
<output>
After completion, create `.planning/phases/19-schema-core-data/19-01-SUMMARY.md`
</output>

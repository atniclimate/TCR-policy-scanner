...
must_haves:
  truths:
    ...
    - "Connecticut planning region FIPS codes (09110-09190) correctly remapped to legacy county codes for crosswalk compatibility"
...
<objective>
Build the NRI expanded metrics extraction pipeline: SHA256 version pinning (XCUT-03), national percentile computation (18.5-DEC-01), EAL breakdown by consequence type, community resilience score, and Connecticut FIPS remapping -- all following the HazardProfileBuilder 9-step template.
...
@.planning/phases/19-schema-core-data/19-RESEARCH.md -- SHA256 code example, national percentile formula, CT FIPS remapping tables, pitfalls 2/3/4/5
@.planning/phases/18.5-architectural-review/sovereignty-scout-report.md -- Confirmed NRI field names: RISK_SCORE, EAL_VALB, EAL_VALP, EAL_VALA, EAL_VALPE, RESL_SCORE, SOVI_SCORE
</context>

<tasks>

<task type="auto">
  <name>Task 1: Implement NRIExpandedBuilder with version pinning and CT FIPS mapping</name>
  ...
  <action>
    ...
    **Step 4: _load_nri_csv() with version pinning (XCUT-03)**
    ...
    - Apply CT FIPS remapping (see below)
    - Return dict[str, dict] keyed by county FIPS

    **Connecticut FIPS remapping (XCUT-03):**
    ```python
    ...
    ```
    At CSV load time:
    - Detect which FIPS format CT uses by checking if any row has STCOFIPS in 09110-09190 range
    - If new-format detected: for each planning region row, create synthetic legacy county entries by copying the data to each mapped legacy FIPS. Log the remapping count.
    - If legacy-format detected: no remapping needed (crosswalk uses legacy codes)
    - This ensures the crosswalk join (which uses legacy county FIPS) always works

    **Step 5: compute_national_percentiles()**
    - From all loaded county data, rank counties by RISK_SCORE ascending
    - Formula: `percentile = rank_0 / (N - 1) * 100` (0-indexed rank)
    - Handle single-county edge case: percentile = 50.0
    - Handle zero-score counties: percentile = 0.0
    ...
    **Step 6: _aggregate_tribe_nri(tribe_id, geoids)**
    ...
    - Normalize weights to matched counties only (existing pattern from hazards.py:616)
    - Compute area-weighted values for: risk_score, risk_percentile (from step 5), eal_total, eal_buildings, eal_population, eal_agriculture, eal_population_equivalence, community_resilience, social_vulnerability_nri
  ...
  <done>
    NRIExpandedBuilder class implements all 9 builder steps: CSV loading with SHA256 validation, CT FIPS remapping, national percentile computation, area-weighted aggregation, atomic writes. All code follows hazards.py patterns exactly.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **TestCTFIPSRemapping:**
    - test_planning_to_legacy_mapping_completeness: All 9 planning regions mapped
    - test_legacy_detection: When CSV has "09001", no remapping occurs
    - test_planning_detection: When CSV has "09110", remapping creates legacy entries
    - test_remapping_preserves_data: Remapped entries have same numeric values as source
    - test_non_ct_fips_untouched: Non-09xxx FIPS are not affected by remapping
    ...
    **TestNRIExpandedBuilder (unit tests with mocked data):**
    ...
    - test_atomic_write_creates_file: Valid write produces JSON file
  ...
  <done>
    Comprehensive test suite for NRI expanded builder with 25+ tests covering CT FIPS remapping, national percentile computation, SHA256 validation, area-weighted aggregation, atomic writes, and path traversal guards. All tests pass.
  </done>
</task>
...
- Connecticut FIPS remapping handles both legacy and planning region formats
...
<output>
After completion, create `.planning/phases/19-schema-core-data/19-04-SUMMARY.md`
</output>

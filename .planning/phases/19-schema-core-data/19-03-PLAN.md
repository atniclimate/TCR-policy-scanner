...
must_haves:
  truths:
    ...
    - "GAP_MESSAGES dict contains all 6 data gap messages from 19-CONTEXT.md"
...
@.planning/phases/19-schema-core-data/19-CONTEXT.md -- ALL user decisions: 5-tier scale, gap messages, SVI Theme 3 exclusion, framing vocabulary
...
<task type="auto">
  ...
  <action>
    ...
    **4. Data gap messages (dict):**
    All 6 messages from 19-CONTEXT.md decisions, keyed by gap type string:
    ```python
    GAP_MESSAGES: dict[str, str] = {
        ...
        "NO_COASTAL_DATA": "",  # Empty: coastal sections omitted entirely for inland Tribes
    ...
    ```
    Note: ALASKA_PARTIAL and SOURCE_UNAVAILABLE have `{placeholders}` for runtime `.format()` substitution.
    ...
    **6. SVI Theme 3 exclusion notes:**
    ...
    ```
    Full note for Doc A, E, C. Short version for Doc B, D.
    ...
    **7. Context-sensitive terms:**
    ...
    ```
    "vulnerability" is allowed ONLY inside these federal proper noun contexts. Not a post-processing filter -- renderers check at construction time.
    ...
    **8. Doc B / Doc D forbidden terms (extends existing air gap):**
    ...
    ```
    These terms must NEVER appear in Doc B or Doc D vulnerability sections.
...
<output>
After completion, create `.planning/phases/19-schema-core-data/19-03-SUMMARY.md`
</output>

...
<task type="auto">
  ...
  <action>
    ...
    **Step 4: _load_svi_csv()**
    ...
    - Extract per county:
      ```python
      {
          ...
          "rpl_themes": _safe_svi_float(row.get("RPL_THEMES")),  # Overall (stored for reference only, includes Theme 3)
      ...
      ```
    - NOTE: Do NOT extract RPL_THEME3. Theme 3 is excluded per user decision.
    ...
    **Step 5: _aggregate_tribe_svi(tribe_id, geoids)**
    ...
    - Skip counties where ALL 3 themes are 0.0 (likely -999 sentinels that were converted)
    ...
    - Determine data_gaps: if coverage_pct == 0 -> MISSING_SVI; if coverage_pct < 0.5 -> partial coverage noted
...
<task type="auto">
  ...
  <action>
    ...
    **TestSVICompositeComputation:**
    ...
    - test_composite_not_rpl_themes: Composite != RPL_THEMES value when Theme 3 != 0
    ...
    **TestSVIProfileBuilder (unit tests):**
    ...
    - test_atomic_write_creates_file: Write produces valid JSON (use tmp_path fixture)
...
<output>
After completion, create `.planning/phases/19-schema-core-data-foundation/19-05-SUMMARY.md`
</output>

...
@.planning/phases/19-schema-core-data-foundation/19-01-SUMMARY.md -- Tech debt fixes + paths + context field
@.planning/phases/19-schema-core-data-foundation/19-02-SUMMARY.md -- Vulnerability Pydantic schemas
@.planning/phases/19-schema-core-data-foundation/19-03-SUMMARY.md -- Vocabulary constants
@.planning/phases/19-schema-core-data-foundation/19-04-SUMMARY.md -- NRI expanded builder
@.planning/phases/19-schema-core-data-foundation/19-05-SUMMARY.md -- SVI builder
...
<task type="auto">
  ...
  <action>
    ...
    **TestDynamicFiscalYear:**
    ...
    - test_no_fy26_in_packet_code: Scan src/packets/ Python files (excluding iija_sunset.py) for "FY26" in non-comment, non-docstring lines. This is a regression guard.
...
<task type="auto">
  ...
  <action>
    ...
    **TestAtomicWriteCompliance:**
    ...
    - test_no_direct_json_dump_to_file: Scan Phase 19 builder files for `json.dump()` calls. Verify that `tempfile` is imported in same file (heuristic for atomic write usage).
    ...
    **TestExistingFileCompliance:**
    ...
    - test_paths_module_no_runtime_checks: Verify src/paths.py does not call .exists() or .mkdir() at import time (constants-only module)
...
<output>
After completion, create `.planning/phases/19-schema-core-data-foundation/19-06-SUMMARY.md`
</output>

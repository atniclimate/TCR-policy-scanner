...
**Issues Resolved:**
- Four-tier crosswalk matching overcame 60/577 initial Tribe-to-district match rate (reached 95.3%)
- python-docx quirks resolved: paragraph.clear() for headers, page breaks not section breaks, OxmlElement per-call freshness
- Security hardening: path traversal validation, JSON cache size limits, temp file ordering
- Organization name references removed from all generated content
- AF-05 ("no web UI") superseded by lightweight static search+download widget
...
- Logger naming inconsistency in economic.py (cosmetic)
- FY26 hardcoded in docx_sections.py (not configurable for future fiscal years)
- Duplicate _format_dollars() in docx_hotsheet.py and docx_sections.py
- Structural asks path hardcoded to data/graph_schema.json (should be in config)
- Some programs missing access_type/funding_type in program_inventory.json
- Award/hazard caches seeded with empty structures; require live API runs to populate
- Phase 8 missing formal VERIFICATION.md (6 summaries serve as equivalent)
- GitHub Pages requires repository configuration (Pages enabled, API key secrets)
- SquareSpace embed URL is placeholder; needs user customization

---

_Archived: 2026-02-10 as part of v1.1 milestone completion_
